 ...
 */
class Version20180428205803 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        switch ($this->connection->getDatabasePlatform()->getName()) {
            case "sqlite":
                $this->addSql('CREATE TABLE actor (id VARCHAR(255) NOT NULL, ctime DATETIME DEFAULT NULL, handle VARCHAR(255) NOT NULL, class VARCHAR(255) NOT NULL, PRIMARY KEY(id))');
                ...
                , title VARCHAR(255) NOT NULL COLLATE BINARY, description CLOB NOT NULL COLLATE BINARY, slug VARCHAR(255) NOT NULL COLLATE BINARY, "order" INTEGER DEFAULT 0 NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_852BBECD727ACA70 FOREIGN KEY (parent_id) REFERENCES forum (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_852BBECDBF396750 FOREIGN KEY (id) REFERENCES estate (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
                ...
                $this->addSql('CREATE TEMPORARY TABLE __temp__grant AS SELECT user_id, attribute, estate_id, is_granted, is_denied FROM grant');
                $this->addSql('DROP TABLE grant');
                $this->addSql('CREATE TABLE grant (estate_id CHAR(36) NOT NULL COLLATE BINARY --(DC2Type:guid)
                , user_id VARCHAR(255) NOT NULL, attribute VARCHAR(255) NOT NULL, is_granted BOOLEAN NOT NULL, is_denied BOOLEAN NOT NULL, PRIMARY KEY(attribute, estate_id, user_id), CONSTRAINT FK_C905664C900733ED FOREIGN KEY (estate_id) REFERENCES estate (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_C905664CA76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
                $this->addSql('INSERT INTO grant (user_id, attribute, estate_id, is_granted, is_denied) SELECT user_id, attribute, estate_id, is_granted, is_denied FROM __temp__grant');
                ...
                , attribute VARCHAR(255) NOT NULL, is_granted_auth BOOLEAN DEFAULT \'0\' NOT NULL, is_granted_anon BOOLEAN DEFAULT \'0\' NOT NULL, is_denied_auth BOOLEAN DEFAULT \'0\' NOT NULL, is_denied_anon BOOLEAN DEFAULT \'0\' NOT NULL, PRIMARY KEY(attribute, estate_id), CONSTRAINT FK_E04992AA900733ED FOREIGN KEY (estate_id) REFERENCES estate (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
                ...
                $this->addSql('INSERT INTO actor SELECT id, ctime, handle, \'user\' AS class FROM user');
                ...
                break;
            case "mysql":
                $this->addSql('CREATE TABLE actor (id VARCHAR(255) NOT NULL, ctime DATETIME DEFAULT NULL, handle VARCHAR(255) NOT NULL, class VARCHAR(255) NOT NULL, PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ENGINE = InnoDB');
                $this->addSql('INSERT INTO actor SELECT id, ctime, handle, \'user\' AS class FROM user');
                $this->addSql('ALTER TABLE user DROP handle, DROP ctime');
    ...
    public function down(Schema $schema)
    {
        switch ($this->connection->getDatabasePlatform()->getName()) {
            case "sqlite":
                ...
                $this->addSql('DROP TABLE "grant"');
                $this->addSql('CREATE TABLE "grant" (estate_id CHAR(36) NOT NULL --(DC2Type:guid)
                , attribute CHAR(36) NOT NULL COLLATE BINARY --(DC2Type:guid)
                , user_id CHAR(36) NOT NULL COLLATE BINARY --(DC2Type:guid)
                , is_granted BOOLEAN NOT NULL, is_denied BOOLEAN NOT NULL, PRIMARY KEY(attribute, estate_id, user_id))');
                ...
                $this->addSql('CREATE TEMPORARY TABLE __temp__permission AS SELECT attribute, estate_id, is_granted_auth, is_granted_anon, is_denied_auth, is_denied_anon FROM permission');
                $this->addSql('DROP TABLE permission');
                $this->addSql('CREATE TABLE permission (estate_id CHAR(36) NOT NULL --(DC2Type:guid)
                , attribute CHAR(36) NOT NULL COLLATE BINARY --(DC2Type:guid)
                ...
                $this->addSql('DROP TABLE __temp__thread');
                $this->addSql('CREATE INDEX IDX_31204C8329CCBAD0 ON thread (forum_id)');
                $this->addSql('ALTER TABLE user ADD COLUMN handle VARCHAR(255) NOT NULL COLLATE BINARY');
                $this->addSql('ALTER TABLE user ADD COLUMN ctime DATETIME DEFAULT NULL');
                $this->addSql('UPDATE `user` SET handle = (SELECT handle FROM actor WHERE id = `user`.id AND class = \'user\'), ctime = (SELECT ctime FROM actor WHERE id = `user`.id AND class = \'user\') WHERE EXISTS (SELECT handle FROM actor WHERE id = `user`.id AND class = \'user\')');
                $this->addSql('DROP TABLE actor');
                break;
            case "mysql":
                $this->addSql('ALTER TABLE user DROP FOREIGN KEY FK_8D93D649BF396750');
                $this->addSql('ALTER TABLE user ADD handle VARCHAR(255) NOT NULL COLLATE utf8mb4_unicode_ci, ADD ctime DATETIME DEFAULT NULL');
                $this->addSql('UPDATE actor INNER JOIN user ON actor.id = user.id AND actor.class = \'user\' SET user.handle = actor.handle, user.ctime = actor.ctime');
                $this->addSql('DROP TABLE actor');
                break;
            default:
                $this->abortIf(true, 'Migration can only be executed safely on \'sqlite\' or \'mysql\'.');
