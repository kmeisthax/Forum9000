 ...
 */
class Version20180426182317 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        switch ($this->connection->getDatabasePlatform()->getName()) {
            case "sqlite":
                ...
                , title VARCHAR(255) NOT NULL COLLATE BINARY, description CLOB NOT NULL COLLATE BINARY, slug VARCHAR(255) NOT NULL COLLATE BINARY, "order" INTEGER DEFAULT 0 NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_852BBECD727ACA70 FOREIGN KEY (parent_id) REFERENCES forum (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
                ...
                break;
            case "mysql":
                $this->addSql('ALTER TABLE user ADD ctime DATETIME DEFAULT NULL');
                break;
            default:
    ...
    public function down(Schema $schema)
    {
        switch ($this->connection->getDatabasePlatform()->getName()) {
            case "sqlite":
                ...
                $this->addSql('DROP TABLE __temp__thread');
                $this->addSql('CREATE INDEX IDX_31204C8329CCBAD0 ON thread (forum_id)');
                $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, handle, password, email, site_role FROM user');
                $this->addSql('DROP TABLE user');
                $this->addSql('CREATE TABLE user (id VARCHAR(255) NOT NULL, handle VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, site_role VARCHAR(255) DEFAULT \'ROLE_USER\' NOT NULL, PRIMARY KEY(id))');
                $this->addSql('INSERT INTO user (id, handle, password, email, site_role) SELECT id, handle, password, email, site_role FROM __temp__user');
                $this->addSql('DROP TABLE __temp__user');
                break;
            case "mysql":
                $this->addSql('ALTER TABLE user DROP ctime');
                break;
            default:
                $this->abortIf(true, 'Migration can only be executed safely on \'sqlite\' or \'mysql\'.');
                break;
